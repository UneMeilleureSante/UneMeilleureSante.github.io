...
switch($request){
	...
	case "include":
		$response = getIncludeScript($queryString);
		saveUserData("../");
		break;
...
function getIncludeScript($queryString){
	if(file_exists("redirect.php"))
		require_once "redirect.php";
	
	preg_match("/\?/", $redirect["grayUrl"], $matches);

	$delimParams = count($matches) ? "&" : "?";
	$newUrl = $redirect["grayUrl"] . ($queryString ? $delimParams . $queryString : "");
	...
	<script>
		function refresh(system){ 
			...
			$.post("php/worked.php", {request: "saveUserData", state: "logged", system: system}, function(data){
